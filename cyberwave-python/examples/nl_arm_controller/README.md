...
- **Planner** — Google Gemini (text-only) → constrained JSON action plan.
...
| Phase | Adds |
|------:|------|
| 2 | Independent smoke tests for SDK + arm, Gemini, Mistral STT, spacebar |
| 3 | Deterministic motion executor (mock JSON plans, no LLM) |
| 4 | Gemini motion planner with constrained JSON output |
